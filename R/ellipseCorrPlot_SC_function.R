...
#' Function to visualize correlation between assay expression
#'
#' Function creates a visualization for assay correlation scores based on ellipse package
...
#' @param lower.panel information to put in lower panels
#' @param diag information to put in diagonal
#' @param ...
                                ...
                                asp = 1, cex.lab = par("cex.lab"), cex = 0.75 * par("cex"), mar = 0.1 + c(2, 2, 4, 2), ...)
{
  ...
  #rowlabs <- factor(as.character(rowlabs),levels = as.character(c("Pou5f1","Cdx2","Gata6","T","Trp53","Nanog")))
  #collabs <- factor(as.character(collabs),levels = as.character(c("Pou5f1","Cdx2","Gata6","T","Trp53","Nanog")))
  
  col <- rgb(mycol((corr+1)/2),maxColorValue=255)
